...
class Model {
    ...
    /**
     * 2015.8.5 very80
     * 对分页的自动处理，支持两种以get方式传递参数的方式：
     * 方式1：传递 page 以直接获取某一页的数据
     * 方式2：传递 lastValue 和 lastType 方式，以获取比 lastValue 大(小) 的一页的数据
     *
     * 注意：两种方式，都必须传递 pageSize 参数以指定页大小
     *
     * @param string $fieldName 用于判断的字段名称，默认为 id
     * @param integer $_size 每页大小
     * @param string  $pagerShow 引用传递的分页结果文本
     *
     * @return Model
     */
    public function pager($conditions, $_size=5, &$pagerShow=null, $fieldName='id') {
        $conditions || $conditions=[];
        // 分页、页大小、上翻、下翻处理
        $_page=I('get.page', I('get.p', '1'));
        $_size=I('get.pageSize', $_size);
        $_lastValue=I('get.lastValue', '');
        $_lastType=I('get.lastType', 'gt');

        if($_lastValue && $_lastType) {
            $fieldName || $fieldName='id';

            $this->where([$fieldName=>[$_lastType, $_lastValue]]);
            $this->limit($_size);
        } elseif($_page && $_size) {
            // 处理分页
            $totalCount = $this->where($conditions)->count();
            $Page = new \Think\Page($totalCount, $_size);
            ...
            // 执行分页查询
            $this->where($conditions)->page($_page, $_size);
        }
